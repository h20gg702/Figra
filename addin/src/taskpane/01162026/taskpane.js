...
if (nrow(comparison_df) > 0) {
  p <- p + stat_pvalue_manual(
    ...
    bracket.size = ${settings.statLineSize || 1.0},
...
async function initWebR() {
  ...
  th + theme(
    ...
    sato_add_statistics_to_plot <- function(p, data, group_col, value_col, test_type="auto", symbol_size=15, show_main_symbol=TRUE, show_pairwise=TRUE, ggpubr_symbol_size=10, ggpubr_line_size=1.2, ggpubr_tip_length=0.04, ggpubr_vjust=-0.3, comparison_mode="significant", custom_comparisons="[]", custom_positions="{}", posthoc_test="tukey", dunnett_control="", y_scale="linear") {
          ...
          if (length(actual_groups) >= 2) {
            ...
            tryCatch({
              ...
              if (length(actual_groups) == 2) {
                ...
                if (omnibus_p_value < 0.001) {
                  real_p_symbols <- c("***")
                } else if (omnibus_p_value < 0.01) {
                  real_p_symbols <- c("**")
                } else if (omnibus_p_value < 0.05) {
                  real_p_symbols <- c("*")
                } else {
                  real_p_symbols <- c("n.s.")
                }
    ...
    sato_box <- function(dat, x_col=1, y_col=NULL, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
      ...
      if (is.null(y_col) || ncol(dat) == 1) {
        ...
        if (add_statistics && length(unique(plot_data$group)) > 1) {
          p <- sato_add_statistics_to_plot(p, plot_data, "group", "value", statistical_test, sato_symbol_size, posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control, y_scale=y_scale)
        }
      }
    ...
    sato_bar_error_dot <- function(dat, x_col=1, y_col=2, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
      ...
      if (add_statistics && length(unique(df$category)) > 1) {
        ...
        p <- sato_add_statistics_to_plot(p, df, "category", "value", statistical_test, sato_symbol_size, show_main_symbol=${showMainStatSymbol ? 'TRUE' : 'FALSE'}, show_pairwise=${showPairwiseComparisons ? 'TRUE' : 'FALSE'}, posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control, y_scale="${yScale}")
      } else {
        if (!add_statistics) {
    ...
    sato_box_dot <- function(dat, x_col=1, y_col=2, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
      ...
      # Add statistical analysis if requested and we have multiple groups
      if (add_statistics && length(unique(df$category)) > 1) {
        p <- sato_add_statistics_to_plot(p, df, "category", "value", statistical_test, sato_symbol_size, posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control, y_scale=y_scale)
      }
...
const fontStack = buildCompleteFontStack(effectiveFont); 
  ...
  window.lastPlotSettings = {
    ...
    statSymbolType, statSymbolSize, statLineSize, statTipLength, statVjust,
