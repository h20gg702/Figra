...
fit <- tryCatch({
  ...
  tryCatch({
    nls(
      response ~ (Top - 0) / (1 + (conc / IC50)^Hill),
      data = dat,
...
curve_data <- data.frame(
  conc = exp(seq(log(min(dat$conc) * 0.5), log(max(dat$conc) * 2), length.out = 200))
)
if (!is.null(fit)) {
...
p <- ggplot(dat, aes(x = conc, y = response)) +
  geom_point(size = ${pointSize}, color = '${pointColor}') +
  scale_x_log10() +
  labs(
...
p <- ggplot(summary_data, aes(x = TimePoint, y = Mean, color = Group, group = Group)) +
  ...
  theme(
    ...
    axis.title.x = element_text(size = ${settings.xAxisTitleSize}, face = '${settings.axisWeight}', margin = margin(t = 6)),
    axis.title.y = element_text(size = ${settings.yAxisTitleSize}, face = '${settings.axisWeight}', margin = margin(r = 6)),
    axis.text.x = element_text(size = ${settings.xAxisTextSize}, color = 'black', face = '${settings.axisWeight}'),
    axis.text.y = element_text(size = ${settings.yAxisTextSize}, color = 'black', face = '${settings.axisWeight}'),
    legend.text = element_text(size = ${settings.legendTextSize})
  )
...
Office.onReady(() => {
  ...
  function updateColorsStyleControlsVisibility() {
    ...
    // Bar Border: Show for single-group charts (including line for line color, NOT IC50)
    if (barBorderRow) {
      const showBarBorder = !isIC50 && singleGroupCharts.includes(chartType);
    ...
    // Bar Width: Show only for bar charts (NOT IC50)
    if (barWidthRow) {
      const showBarWidth = !isIC50 && barCharts.includes(chartType);
...
async function initWebR() {
  ...
  th + theme(
    ...
    sato_line_grouped_error_raw <- function(dat, group_col=1, x_col=2, y_col=3,
      ...
      p <- sato_apply_theme(p, target_font, title_weight, axis_weight,
                      title_size, x_axis_title_size, y_axis_title_size, x_axis_text_size, y_axis_text_size, legend_text_size,
                      title_text, x_text, y_text,
                      show_title, show_x_label, show_y_label,
                      x_scale, y_scale,
    ...
    sato_ic50 <- function(dat, x_col=1, y_col=2,
                          ...
                          point_size=3, point_color="#1f2937",
      ...
      if (fit_success && !is.null(fit)) {
        conc_seq <- 10^seq(log10(min(plot_data$conc)), log10(max(plot_data$conc)), length.out = 100)
      ...
      p <- ggplot(plot_data, aes(x = conc, y = response)) +
        geom_point(size = point_size, color = point_color, alpha = 0.8)
      ...
      # Store IC50 result for UI display
...
const fontStack = buildCompleteFontStack(effectiveFont); 
  ...
  console.log("IC50 settings:", { enableIC50Analysis, showIC50Value, showIC50Line, showConfidenceBand, ic50CurveColor, ic50CurveWidth, ic50LineColor, ic50PointSize, ic50PointColor });
  ...
  window.lastPlotSettings = {
    ...
    ic50CurveColor, ic50CurveWidth, ic50LineColor, ic50PointSize, ic50PointColor
  };
