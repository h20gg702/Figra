...
// Extract only relevant functions from full R code for the specific chart type
// New approach: Build minimal R code from scratch instead of parsing
function extractRelevantRCode() {
  try {
    ...
    console.log(`Building minimal R code for chart type: ${chartType}`);

    // Step 1: Extract libraries section (always the same)
    const librariesMatch = fullCode.match(/(# Load required libraries[\s\S]*?)(?=\n# )/);
    const libraries = librariesMatch ? librariesMatch[1].trim() : `# Load required libraries
library(ggplot2)
library(dplyr)
library(ggpubr)`;

    // Step 2: Extract data frame section
    const dataFrameMatch = fullCode.match(/(# Create data frame[\s\S]*?dat <- data\.frame\([\s\S]*?\)\s*\n)/);
    const dataFrame = dataFrameMatch ? dataFrameMatch[1].trim() : '';

    // Step 3: Extract helper functions (always needed)
    const helperFunctions = extractFunctionsByName(fullCode, [
      'sato_theme',
      'sato_apply_theme',
    ...
    // Step 4: Extract the specific chart function used
    const chartFunctionName = `sato_${chartType}`;
    const chartFunction = extractFunctionsByName(fullCode, [chartFunctionName]);

    // Step 5: Extract the specific function call and plot execution
    const functionCallMatch = fullCode.match(new RegExp(
      `(if \\(chart_type == "${chartType}"\\) \\{[\\s\\S]*?p <- ${chartFunctionName}\\([\\s\\S]*?\\)\\s*\\})`
    ));

    let plotExecution = '';
    if (functionCallMatch) {
      // Extract just the plot call without the if statement
      const ifBlockContent = functionCallMatch[1];
      const plotCallMatch = ifBlockContent.match(/(p <- sato_[\s\S]*?\)(?=\s*\}))/);
      if (plotCallMatch) {
        plotExecution = `# Create the plot\nchart_type <- "${chartType}"\n\n${plotCallMatch[1]}`;
      }
    }

    // If we couldn't extract the plot call, try a simpler match
    if (!plotExecution) {
      const simplePlotMatch = fullCode.match(new RegExp(
        `(p <- ${chartFunctionName}\\([\\s\\S]*?\\))`
      ));
      if (simplePlotMatch) {
        plotExecution = `# Create the plot\nchart_type <- "${chartType}"\n\n${simplePlotMatch[1]}`;
      }
    ...
    // Step 6: Extract label application and print statement
    const labelsMatch = fullCode.match(/(# Apply formatted labels after plot creation[\s\S]*?print\(p\))/);
    const labels = labelsMatch ? labelsMatch[1].trim() : 'print(p)';

    // Build the final minimal R code
    const minimalCode = `${libraries}
...
# ============ Apply Labels and Display ============

${labels}
`;

    console.log(`✅ Generated minimal R code (${minimalCode.length} chars vs ${fullCode.length} chars)`);
    return minimalCode;

  } catch (e) {
    console.error('Extract relevant R code failed:', e);
    console.error('Error details:', e.stack);
    // Fallback to full code
    return window.lastRCode || `# Error: ${e.message}`;
  }
}

// Helper function to extract specific functions from R code
function extractFunctionsByName(code, functionNames) {
  const functions = [];

  for (const funcName of functionNames) {
    // Find the function definition start
    const functionStartPattern = new RegExp(`${funcName}\\s*<-\\s*function`, 'i');
    const lines = code.split('\n');

    let foundStart = false;
    let braceCount = 0;
    let completeFuncLines = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!foundStart && functionStartPattern.test(line)) {
        foundStart = true;
        console.log(`Found function: ${funcName} at line ${i}`);
      }

      if (foundStart) {
        completeFuncLines.push(line);
        ...
        // If we've closed all braces and we had at least one open brace
        if (braceCount === 0 && hasOpenedBrace) {
          console.log(`Completed extracting ${funcName} (${completeFuncLines.length} lines)`);
          functions.push(`# ${funcName} function\n${completeFuncLines.join('\n')}`);
          break;
        }
      }
    }

    // If we didn't find the function, log a warning
    if (!foundStart) {
      console.warn(`Function ${funcName} not found in R code`);
    } else if (!hasOpenedBrace) {
      console.warn(`Function ${funcName} found but no braces detected`);
    }
  }
    ...
    sato_line_grouped_error_raw <- function(dat, group_col=1, x_col=2, y_col=3,
      ...
      if (add_statistics && n_groups >= 2) {
        ...
        if (n_groups == 2 && nrow(stat_results) > 0) {
          ...
          cat("Multi-group line plot: Statistical results printed to console only (no visual symbols yet)\\n")
        }
...
async function loadHeadersFromSelection(){
  try {
    ...
    if (GROUPED_CHART_TYPES.includes(chartType) && headers.length >= 3) {
      ...
      // For charts with error column (4+ columns): Error=4th column
      if (chartType === "line_grouped_error" && headers.length >= 4) {
        if (errorSel) errorSel.value = headers[3];  // 4th column = Error
      }
    } else {
      // For regular charts
...
async function insertIntoExcelFixed() {
  try {
    ...
    await Excel.run(async (ctx) => {
      const ws = ctx.workbook.worksheets.getActiveWorksheet();
      const img = ws.shapes.addImage(b64);
    ...
    setStatus(
      `Excel挿入完了: ${meta.width}${meta.units} × ${meta.height}${meta.units}\n` +
