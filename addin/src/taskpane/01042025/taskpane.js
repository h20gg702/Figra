    ...
    input.onchange = async (e) => {
      try {
        ...
        if (!metadata.addinVersion) {
          versionMessage = `⚠️ Version info: This figure was created with an older version of OPEN FIGURE (before version tracking).\nCurrent version: ${ADDIN_VERSION}\n\nThe figure may not reproduce exactly. Proceed with caution.\n\n`;
...
function generateSubsetRCodeFromData(chartType, opts) {
  try {
    ...
    ggplotCode += `  axis.text.x = element_text(size = ${xAxisTextSize}, family = '${fontFamily}', color = 'black', face = '${axisWeight}'),\n`;
    ggplotCode += `  axis.text.y = element_text(size = ${yAxisTextSize}, family = '${fontFamily}', color = 'black', face = '${axisWeight}'),\n`;
...
p <- ggplot(dat, aes(x = dat[[${settings.xColIndex || 1}]], y = dat[[${settings.yColIndex || 2}]])) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = ${settings.xAxisTextSize}, color = 'black', face = '${settings.axisWeight}'),
    axis.text.y = element_text(size = ${settings.yAxisTextSize}, color = 'black', face = '${settings.axisWeight}'),
...
`;
  ...
  // Sanitize column names to match data frame creation (replace non-alphanumeric with _)
  const groupColName = headers[groupIdx] ? String(headers[groupIdx]).replace(/[^a-zA-Z0-9_]/g, '_') : 'Treatment';
  const yColName = headers[yIdx] ? String(headers[yIdx]).replace(/[^a-zA-Z0-9_]/g, '_') : 'Value';
...
group1_data <- cat_data_${safeName}[cat_data_${safeName}$${groupColName} == '${groups[0]}', '${yColName}']
group2_data <- cat_data_${safeName}[cat_data_${safeName}$${groupColName} == '${groups[1]}', '${yColName}']
...
# Add significance symbols to plot using annotate
`;
  ...
  // Generate annotation code for each category
  // For grouped box plots, we need to draw brackets between groups
  ...
  // Pre-calculate Y positions for all categories (simpler educational code)
  ...
  categories.forEach(category => {
    ...
    // Calculate Y position: either custom or automatic
    let yBracketPos;
    if (customYPos) {
    ...
    // Generate simplified bracket code with pre-calculated Y position
    code += `
# Add bracket and annotation for ${category}
x_left_${safeCategoryName} <- ${catIndex} - ${dodgeWidth}/4
x_right_${safeCategoryName} <- ${catIndex} + ${dodgeWidth}/4
y_bracket_${safeCategoryName} <- ${yBracketPos}

# Draw bracket lines
p <- p + annotate('segment', x = x_left_${safeCategoryName}, xend = x_left_${safeCategoryName},
                  y = y_bracket_${safeCategoryName} - 0.02, yend = y_bracket_${safeCategoryName},
                  linewidth = 0.5, color = 'black')
p <- p + annotate('segment', x = x_left_${safeCategoryName}, xend = x_right_${safeCategoryName},
                  y = y_bracket_${safeCategoryName}, yend = y_bracket_${safeCategoryName},
                  linewidth = 0.5, color = 'black')
p <- p + annotate('segment', x = x_right_${safeCategoryName}, xend = x_right_${safeCategoryName},
                  y = y_bracket_${safeCategoryName} - 0.02, yend = y_bracket_${safeCategoryName},
                  linewidth = 0.5, color = 'black')

# Add significance text above bracket
p <- p + annotate('text', x = ${catIndex}, y = y_bracket_${safeCategoryName} + 0.02,
                  label = sig_${safeCategoryName}, size = ${settings.statSymbolSize || 7})
`;
  });
            ...
            if (n_groups_at_cat == 2) {
              ...
              if (length(group1_data) > 0 && length(group2_data) > 0) {
                ...
                if (!is.null(test_result)) {
                  ...
                  # Store result text for UI with proper order: Category -> Normality -> Variance -> Test result
                  ...
                  result_text <- paste0(result_text, "\\n", sprintf("%s vs %s, %s p=%.4f (%s)",
                                        groups_with_data[1], groups_with_data[2],
                                        test_name, p_val, sig_label))
                  stat_text_results <- c(stat_text_results, result_text)
