...
function collectCurrentSettings() {
  ...
  const settings = {
    ...
    vbracketMargin: el("vbracketMargin")?.value || "0.03",
...
Office.onReady(() => {
  ...
  function updateColorsStyleControlsVisibility() {
    ...
    // Bar Interior: Show for single-group charts (NOT grouped, NOT line)
    if (barInteriorRow) {
      const showBarInterior = singleGroupCharts.includes(chartType) && chartType !== 'line';
      barInteriorRow.style.display = showBarInterior ? "flex" : "none";
    }

    // Bar Border: Show for single-group charts (including line for line color)
    if (barBorderRow) {
      const showBarBorder = singleGroupCharts.includes(chartType);
      barBorderRow.style.display = showBarBorder ? "flex" : "none";

      // Update label text for line chart
    ...
    // Line Width: Always show (used by all chart types)
    if (lineWidthRow) {
      lineWidthRow.style.display = "flex";
    }

    // Bar Width: Show only for bar charts
    if (barWidthRow) {
      const showBarWidth = barCharts.includes(chartType);
      barWidthRow.style.display = showBarWidth ? "flex" : "none";
    }

    // Dodge Width: Show only for grouped charts with dodge (NOT line_grouped*)
    if (dodgeWidthRow) {
      const showDodgeWidth = groupedWithDodge.includes(chartType);
      dodgeWidthRow.style.display = showDodgeWidth ? "flex" : "none";
    }

    // Dot Settings: Show only for charts with dots
    if (dotSettingsRow) {
      const showDotSettings = dotCharts.includes(chartType);
...
async function initWebR() {
  ...
  th + theme(
    ...
    sato_line_grouped_error_raw <- function(dat, group_col=1, x_col=2, y_col=3,
                                            ...
                                            vbracket_text_size=14, vbracket_margin=0.03, vbracket_line_width=0.5,
...
const fontStack = buildCompleteFontStack(effectiveFont); 
  ...
  const vbracketMargin = Number(o.vbracketMargin) || 0.03;
  const vbracketLineWidth = Number(o.vbracketLineWidth) || 0.5;
  console.log("VBracket settings:", { vbracketTimepoint, vbracketPosition, vbracketX, vbracketY, vbracketTextSize, vbracketMargin, vbracketLineWidth });
  ...
  window.lastPlotSettings = {
    ...
    vbracketTimepoint, vbracketPosition, vbracketX, vbracketY, vbracketTextSize, vbracketMargin, vbracketLineWidth,
...
function uiOpts(){
  ...
  return {
    ...
    vbracketMargin: Number(el("vbracketMargin")?.value) || 0.03,
