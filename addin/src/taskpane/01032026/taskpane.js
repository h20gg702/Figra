...
async function exportStatisticalResultsToNewSheet() {
  try {
    ...
    if (chartType === "line_grouped_error_raw") {
      statResults = await getLineStatisticalResultsText();
    } else if (chartType === "bar_grouped" || chartType === "bar_grouped_error" || chartType === "bar_grouped_error_dot" || chartType === "box_grouped_dot" || chartType === "violin_grouped_dot") {
      statResults = await getGroupedBarStatisticalResultsText();
    } else {
    ...
    if (!statResults || statResults.includes("No statistical") || statResults.includes("ERROR") || statResults.includes("Statistics disabled")) {
      setStatus(`Statistical results not available: ${statResults ? statResults.substring(0, 100) : "null"}`);
      return;
    }
  ...
  const plotCode = `
    ...
    if (chart_type == "histogram") {
      ...
      p <- sato_violin_grouped_dot(
        ...
        group_name = "${selectedGroupColumn || 'Group'}",
        category_name = "${selectedXColumn || 'Category'}",
        value_name = "${selectedYColumn || 'Value'}",
        target_font = target_font,
        title_weight = title_weight,
        axis_weight = axis_weight,
        title_size = ${titleSize},
...
async function previewPlotWithDebug() {
  try {
    ...
    img.onload = () => {
      ...
      // Auto-export statistical results to new sheet if statistics are enabled
