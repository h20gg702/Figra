  ...
  try {
    ...
    if (addStatistics && isChartTypeSupported) {
      ...
      try {
        ...
        } else if (chartType === "bar_grouped_error_dot") {
    ...
    input.onchange = async (e) => {
      try {
        ...
        if (file.name.endsWith('.json')) {
          ...
          // Check if this is an OPEN FIGURE PNG
          if (!metadata || !metadata.data) {
            setStatus("❌ This PNG was not created by OPEN FIGURE (no metadata found)\n\nTip: Use '📦 Download Metadata (JSON)' button to save metadata separately!");
            return;
          }
        }
        ...
        const statMsg = metadata.statisticalResults ? "\n📊 Statistical results included in metadata" : "";
        setStatus(`✅ Loaded from ${file.name} (created: ${createdDate})${statMsg} - Click Preview to verify reproducibility`);
  ...
  try {
    ...
    if (hasHeader) {
      ...
      headers.forEach((header, colIdx) => {
        const colName = String(header).replace(/[^a-zA-Z0-9_]/g, '_');
    ...
    const axisWeight = document.getElementById("axisWeight")?.value || "plain";

    switch(chartType) {
      case 'histogram':
        ggplotCode += `p <- ggplot(dat, aes(x = dat[[1]])) +\n`;
        ggplotCode += `  geom_histogram(bins = 20, fill = '${fillColor || '#4C78A8'}', color = '${strokeColor || '#1f2937'}', alpha = ${fillAlpha || 1}) +\n`;
        ggplotCode += `  labs(title = '${title || 'Histogram'}', x = '${xlab || 'Value'}', y = '${ylab || 'Frequency'}') +\n`;
        ggplotCode += `  theme_${themeName || 'classic'}(base_size = ${xAxisTextSize}, base_family = '${fontFamily}')\n`;
        break;

      case 'bar':
      case 'bar_error':
        ggplotCode += `p <- ggplot(dat, aes(x = dat[[1]], y = dat[[2]])) +\n`;
        ...
        ggplotCode += `  labs(title = '${title || 'Bar Plot'}', x = '${xlab || 'Category'}', y = '${ylab || 'Value'}') +\n`;
        ggplotCode += `  theme_${themeName || 'classic'}(base_size = ${xAxisTextSize}, base_family = '${fontFamily}')\n`;
        break;
      ...
      case 'bar_grouped_error_dot':
        ...
        ggplotCode += `p <- ggplot(dat, aes(x = dat[[${xColIndex || 2}]], y = dat[[${yColIndex || 3}]], fill = dat[[${groupColIndex || 1}]])) +\n`;
        ...
        ggplotCode += `  labs(title = '${title || 'Grouped Bar Plot'}', x = '${xlab || 'Treatment'}', y = '${ylab || 'Value'}', fill = 'Group') +\n`;
        ggplotCode += `  theme_${themeName || 'classic'}(base_size = ${xAxisTextSize}, base_family = '${fontFamily}')\n`;
        break;
      ...
      case 'line_grouped_error_raw':
        ...
        ggplotCode += `p <- ggplot(dat, aes(x = dat[[${xColIndex || 2}]], y = dat[[${yColIndex || 3}]], color = dat[[${groupColIndex || 1}]])) +\n`;
        ggplotCode += `  geom_point(size = 3) +\n`;
        ggplotCode += `  scale_color_manual(values = ${groupColorsLine}) +\n`;
        ggplotCode += `  labs(title = '${title || 'Grouped Plot'}', x = '${xlab || 'X'}', y = '${ylab || 'Value'}', color = 'Group') +\n`;
        ggplotCode += `  theme_${themeName || 'classic'}(base_size = ${xAxisTextSize}, base_family = '${fontFamily}')\n`;
        break;
      ...
      case 'box_dot':
        ...
        ggplotCode += `  labs(title = '${title || 'Box Plot'}', x = '${xlab || 'Group'}', y = '${ylab || 'Value'}') +\n`;
        ggplotCode += `  theme_${themeName || 'classic'}(base_size = ${xAxisTextSize}, base_family = '${fontFamily}')\n`;
        break;
      ...
      case 'dot':
        ...
        ggplotCode += `  labs(title = '${title || 'Dot Plot'}', x = '${xlab || 'X'}', y = '${ylab || 'Y'}') +\n`;
        ggplotCode += `  theme_${themeName || 'classic'}(base_size = ${xAxisTextSize}, base_family = '${fontFamily}')\n`;
        break;
    ...
    // Add detailed theme customization for text sizes
    ggplotCode += `\n# Customize text sizes\n`;
    ggplotCode += `p <- p + theme(\n`;
    ggplotCode += `  plot.title = element_text(size = ${titleSize}, face = '${titleWeight}', family = '${fontFamily}'),\n`;
    ggplotCode += `  axis.title.x = element_text(size = ${xAxisTitleSize}, face = '${axisWeight}', family = '${fontFamily}'),\n`;
    ggplotCode += `  axis.title.y = element_text(size = ${yAxisTitleSize}, face = '${axisWeight}', family = '${fontFamily}'),\n`;
    ggplotCode += `  axis.text.x = element_text(size = ${xAxisTextSize}, family = '${fontFamily}'),\n`;
    ggplotCode += `  axis.text.y = element_text(size = ${yAxisTextSize}, family = '${fontFamily}'),\n`;
...
`;
  ...
  if (chartType === 'bar_grouped_error_dot' || chartType === 'bar_grouped_error' || chartType === 'bar_grouped') {
    ...
    code += generateLineGroupedCode(settings, groupColors);
             ...
             chartType.startsWith('box_grouped') || chartType.startsWith('violin_grouped')) {
    // For other chart types, provide a simplified code template
    code += `# Chart type '${chartType}' - Simplified template
# This is a basic template. For exact reproduction, more code generation is needed.

p <- ggplot(dat, aes(x = dat[[${settings.xColIndex || 1}]], y = dat[[${settings.yColIndex || 2}]])) +
  geom_point() +
  theme_${settings.themeName}(base_family = '${settings.fontFamily}') +
  theme(
    plot.title = element_text(size = ${settings.titleSize}, face = '${settings.titleWeight}'),
    axis.title.x = element_text(size = ${settings.xAxisTitleSize}),
    axis.title.y = element_text(size = ${settings.yAxisTitleSize}),
    axis.text.x = element_text(size = ${settings.xAxisTextSize}),
    axis.text.y = element_text(size = ${settings.yAxisTextSize})
  )
...
function generateGroupedBarCode(chartType, settings, groupColors) {
  ...
  const colNames = window.lastProcessedData && window.lastProcessedData.length > 0
    ? window.lastProcessedData[0].map(h => String(h).replace(/[^a-zA-Z0-9_]/g, '_'))
    : ['Group', 'Category', 'Value'];

  const groupColName = colNames[settings.groupColIndex - 1] || 'Group';
  const xColName = colNames[settings.xColIndex - 1] || 'Category';
  const yColName = colNames[settings.yColIndex - 1] || 'Value';

  let code = `# ============ Calculate Summary Statistics ============
...
`;
  ...
  theme(
    plot.title = element_text(size = ${settings.titleSize}, face = '${settings.titleWeight}'),
    axis.title.x = element_text(size = ${settings.xAxisTitleSize}, face = '${settings.axisWeight}'),
    axis.title.y = element_text(size = ${settings.yAxisTitleSize}, face = '${settings.axisWeight}'),
    axis.text.x = element_text(size = ${settings.xAxisTextSize}),
    axis.text.y = element_text(size = ${settings.yAxisTextSize}),
...
#
# library(ggpubr)
# comparisons <- list(c("Group1", "Group2"), c("Group1", "Group3"))
# p <- p + stat_compare_means(comparisons = comparisons, method = "t.test")
#
# Uncomment the lines above and modify the comparisons list as needed.
...
function generateLineGroupedCode(settings, groupColors) {
  ...
  const groupColName = colNames[settings.groupColIndex - 1] || 'Group';
  const xColName = colNames[settings.xColIndex - 1] || 'TimePoint';
  const yColName = colNames[settings.yColIndex - 1] || 'Value';
...
p <- ggplot(summary_data, aes(x = TimePoint, y = Mean, color = Group, group = Group)) +
  ...
  theme(
    ...
    axis.title.x = element_text(size = ${settings.xAxisTitleSize}),
    axis.title.y = element_text(size = ${settings.yAxisTitleSize}),
    axis.text = element_text(size = ${settings.xAxisTextSize}),
    legend.text = element_text(size = ${settings.legendTextSize})
  )
...
async function writeRCodeToCellHandler() {
  try {
    ...
    console.error('Write R code error:', e);
    setStatus("Error: " + (e?.message || e));
  }
}
...
function collectCurrentSettings() {
  ...
  const settings = {
    ...
    axisWeight: el("axisWeight")?.value || "plain",
    ...
    symbolType: el("symbolType")?.value || "star",
...
function applySettingsToUI(settings) {
  ...
  setValue("axisWeight", settings.axisWeight);
  ...
  // VBracket legend settings
  setValue("vbracketPosition", settings.vbracketPosition);
...
Office.onReady(() => {
  ...
  function updateVbracketPositionInputs() {
    const vbracketPosition = document.getElementById("vbracketPosition")?.value || "";
    const vbracketXRow = document.getElementById("vbracketXRow");
    const vbracketYRow = document.getElementById("vbracketYRow");

    if (!vbracketXRow || !vbracketYRow) return;

    // Show X/Y inputs only when "custom" is selected
    const shouldShow = vbracketPosition === "custom";
    vbracketXRow.style.display = shouldShow ? "flex" : "none";
    vbracketYRow.style.display = shouldShow ? "flex" : "none";
  }
  ...
  function handleChartTypeChange() {
    ...
    if (v === "box") {
      lx.textContent = "Group (optional)";
      ly.textContent = "Y Axis";
    } else if (v === "bar") {
      lx.textContent = "Category (text)";
      ly.textContent = "Y Axis (auto: Count)";
    } else if (isGrouped) {
      lx.textContent = "X Axis (category/numeric)";
      ly.textContent = "Y Axis";
  ...
  // Set up vbracket position dropdown listener to show/hide X/Y inputs
  document.getElementById("vbracketPosition")?.addEventListener("change", updateVbracketPositionInputs);
...
function populateComparisonCheckboxes() {
  ...
  // Check if this is a line plot with time series or grouped bar with categories
  const chartType = document.getElementById("chartType")?.value || "";
  const isLinePlot = chartType === "line_grouped_error_raw" || chartType === "line_grouped_error";
  const isGroupedBar = chartType === "bar_grouped_error_dot";
  ...
  if (isLinePlot || isGroupedBar) {
    // For line plots: generate comparisons for each time point
    // Get unique X values (time points) from the data
    if (window.lastProcessedData && window.lastProcessedData.length > 1) {
      const headers = window.lastProcessedData[0];
      const selectedXColumn = document.getElementById("xColumn")?.value || "";
  ...
  combinations.forEach((comp, index) => {
    ...
    if (comp.length === 3) {
      // Line plot: "Group1 vs Group2 @ Day X"
      const xColumnName = document.getElementById("xColumn")?.value || "X";
      text.textContent = `${comp[0]} vs ${comp[1]} @ ${xColumnName} ${comp[2]}`;
    } else {
    ...
    positionInput.min = "50";
    positionInput.max = "500";
    positionInput.step = "5";

    // Calculate default absolute Y position
    let defaultAbsoluteY = Math.round(60 + index * 15); // Fallback: Start at 60, increment by 15

    // For line plots and grouped bar charts, calculate actual position based on data
    if ((isLinePlot || isGroupedBar) && comp.length === 3 && window.lastProcessedData && window.lastProcessedData.length > 1) {
      const headers = window.lastProcessedData[0];
      ...
      if (groupIdx >= 0 && xIdx >= 0 && yIdx >= 0) {
        ...
        if (isGroupedBar) {
          // For grouped bar: find maximum bar height across ALL groups at this X value
          let maxBarTop = 0;
          ...
          if (maxBarTop > 0) {
            // Position ABOVE the tallest bar + error with spacing
            defaultAbsoluteY = Math.round(maxBarTop + (maxBarTop * 0.05));
          }
        } else {
  ...
  const detailedTestResult = await webR.evalR(`
    ...
    sato_check_normality <- function(data, group_col=NULL) {
      if (is.null(group_col)) {
        ...
        shapiro_result <- shapiro.test(data[[ncol(data)]])
        return(list(
        ...
        for (group in groups) {
          ...
          if (nrow(group_data) >= 3) {
            shapiro_result <- shapiro.test(group_data[[ncol(group_data)]])
    ...
    if (exists("dat") && is.data.frame(dat) && ncol(dat) >= 2) {
      group_col <- dat[[1]]
      value_col <- dat[[2]]
      ...
      if (length(groups) >= 2) {
        ...
        # Add variance test results (for 2-group comparisons with t-test)
        variance_text <- ""
        ...
        posthoc_results <- ""
        if (grepl("ANOVA", test_name) && p_val < 0.05 && length(groups) > 2) {
          cat("Performing", selected_posthoc_test, "post-hoc test for display\\n")
          ...
          tryCatch({
            ...
            if (selected_posthoc_test == "tukey") {
              ...
              # For other post-hoc tests, show generic message
        ...
        full_result <- paste0(main_result, normality_text, variance_text, group_stats, posthoc_results)
...
async function displayStatisticalResultsInUI() {
  try {
    ...
    // Handle grouped bar with dots (category-specific comparisons)
    if (chartType === "bar_grouped_error_dot") {
...
async function exportStatisticalResults() {
  try {
    ...
    if (chartType === "line_grouped_error_raw") {
      ...
      // For grouped bar charts, get the detailed statistical results text
      const groupedBarStatResults = await getGroupedBarStatisticalResultsText();
    ...
    const detailedTestResult = await webR.evalR(`
      cat("=== COMPREHENSIVE STATISTICAL ANALYSIS FOR EXPORT ===\\n")
      ...
      sato_perform_statistical_test <- function(data, group_col, value_col, test_type="auto", variance_test="levene", posthoc_test="tukey") {
        ...
        # For 3+ groups
        if (test_type == "anova") {
          result <- aov(as.formula(paste(names(data)[value_col], "~", names(data)[group_col])), data=data)
          ...
          result <- kruskal.test(as.formula(paste(names(data)[value_col], "~", names(data)[group_col])), data=data)
          return(list(
            p_value = result$p.value,
      ...
      if (!exists("dat") || !is.data.frame(dat) || nrow(dat) < 2) {
        ...
        # For your Control/Treatment data
        if (ncol(dat) >= 2) {
          group_col <- dat[[1]]  # First column (Group)
          value_col <- as.numeric(dat[[2]])  # Second column (Value)
          ...
          if (length(groups) == 2) {
            ...
            if (length(group1_values) >= 3 && length(group1_values) <= 5000 &&
              ...
              if ("` + (statisticalTest || 'auto') + `" == "auto") {
                ...
                test_used <- "` + (statisticalTest || 'auto') + `"
                cat("Manual selection:", test_used, "\\n")
              }
            } else {
              cat("Sample size too small for normality testing (using t-test)\\n")
              test_used <- if ("` + (statisticalTest || 'auto') + `" == "auto") "t-test" else "` + (statisticalTest || 'auto') + `"
            ...
            cat("🔥🔥🔥 MULTIPLE GROUPS DETECTED - SHOULD REACH TUKEY HSD SECTION 🔥🔥🔥\\n")
            cat("Multiple groups detected - performing overall test\\n")
            ...
            if (grepl("ANOVA", test_name) && p_val < 0.05) {
              ...
              tryCatch({
                ...
                cat("Post-hoc test completed successfully\\n")
                ...
                cat(selected_posthoc_test, "post-hoc error:", e$message, "\\n")
              })
            } else if (p_val >= 0.05) {
              posthoc_results <- "\\n\\nNo post-hoc test performed (overall test not significant)"
            } else {
            ...
            cat("\\nFINAL COMPREHENSIVE RESULT FOR MULTIPLE GROUPS:\\n")
            cat(full_result, "\\n")
            full_result
          } else {
            "ERROR: Need at least 2 groups"
...
async function initWebR() {
  ...
  if (!WebR) { setStatus("WebR load failed (check <script type=module> in HTML)"); return; }
  ...
  title_face="plain", axis_face="plain",
  ...
  th + theme(
    text               = element_text(family = family),   # ← これを追加
    ...
    axis.title.x = element_text(family=family, face=axis_face, size=x_axis_title_size, margin=margin(t=6)),
    axis.title.y = element_text(family=family, face=axis_face, size=y_axis_title_size, margin=margin(r=6)),
    axis.text.x  = element_text(family=family, face=axis_face, size=x_axis_text_size, color="black"),
    axis.text.y  = element_text(family=family, face=axis_face, size=y_axis_text_size, color="black"),
    ...
    sato_add_statistics_to_plot <- function(p, data, group_col, value_col, test_type="auto", symbol_size=15, show_main_symbol=TRUE, show_pairwise=TRUE, ggpubr_symbol_size=10, ggpubr_line_size=1.2, ggpubr_tip_length=0.04, ggpubr_vjust=-0.3, comparison_mode="significant", custom_comparisons="[]", custom_positions="{}", posthoc_test="tukey", dunnett_control="") {
        ...
        tryCatch({
          ...
          if (length(actual_groups) >= 2) {
            ...
            tryCatch({
              ...
              # Step 1: Determine which omnibus test to use (ANOVA or Kruskal-Wallis)
              ...
              # Step 2: Perform omnibus test
              if (omnibus_test_type == "anova") {
              ...
              if (omnibus_p_value >= 0.05) {
                ...
                for (i in 1:length(combinations)) {
                  ...
                  if (p_val < 0.001) real_p_symbols <- c(real_p_symbols, "***")
                  else if (p_val < 0.01) real_p_symbols <- c(real_p_symbols, "**")
                  else if (p_val < 0.05) real_p_symbols <- c(real_p_symbols, "*")
                  else real_p_symbols <- c(real_p_symbols, "n.s.")

                  cat("🔥", combinations[[i]][1], "vs", combinations[[i]][2], ": p =", p_val, "→", tail(real_p_symbols, 1), "🔥\\n")
                }
              }  # Close the omnibus significance check (else block)

              # Calculate unit_step as a fraction of y_range
              calculate_unit_step <- function(symbols, symbol_size, y_range, num_comparisons) {
                ...
                # Calculate unit_step as fraction * y_range
                ...
                return(max(unit_step, y_range * 0.08))  # Minimum 8% of range
              }

              # Calculate the dynamic step for this data (unit_step)
              y_range <- y_max - min(data[[value_col]], na.rm = TRUE)
              num_comparisons <- length(combinations)
              unit_step <- calculate_unit_step(real_p_symbols, ggpubr_symbol_size, y_range, num_comparisons)
              cat("🔥 CALCULATED unit_step =", unit_step, "for y_range =", y_range, ", num_comparisons =", num_comparisons, "🔥\\n")
              ...
              for (idx in seq_along(combinations_for_positioning)) {
                ...
                if (!is.null(custom_pos) && is.numeric(custom_pos)) {
                  ...
                  # First bracket: baseHeight + smaller offset for compactness
                  # Subsequent brackets: max(baseHeight + unit_step, previous_bracket + unit_step)
                  if (idx == 1) {
              ...
              if (comparison_mode == "significant") {
                ...
                if (length(significant_indices) > 0) {
                  ...
                  if (!has_custom_pos) {
                    ...
                    unit_step_filtered <- calculate_unit_step(pairwise_data$p.signif, ggpubr_symbol_size, y_range, num_filtered)
                    ...
                    for (i in 1:num_filtered) {
                      ...
                      # Calculate position with consistent spacing using filtered unit_step
                      if (i == 1) {
          ...
          cat("🔥 Y-axis limit: max_bracket=", max_bracket_y, " + ", expansion_factor, "*y_range(", y_range, ") = ", y_axis_upper_limit, "🔥\\n")
          ...
          if (show_main_symbol && length(actual_groups) > 2) {
            ...
            if (omnibus_sig_symbol != "ns") {
              ...
              # Position should be ABOVE the highest bracket
              # max_bracket_y was set earlier - use that + some space
              ...
              # Expand y-axis to accommodate the overall symbol
          ...
          p <- p + annotate("text", x = 3, y = max(data[[value_col]], na.rm = TRUE) * 1.1,
                           label = paste("GGPUBR ERROR:", e$message), color = "red", size = 3)
        })
      }
      ...
      tryCatch({
        ...
        cat("🔥🔥🔥 FORCED GGPUBR: Enabling ggpubr regardless of conditions for debugging 🔥🔥🔥\\n")
        use_ggpubr <- TRUE  # Force enable for debugging
        ...
        if (use_ggpubr) {
          ...
          if (length(groups) == 2) {
            ...
            if (our_significance != "ns") {
              ...
              y_max <- max(data[[value_col]], na.rm = TRUE)
              y_pos <- y_max * 1.15  # Position above the highest data point
              ...
              # Add bracket lines
              p <- p + 
                annotate("segment", x = 1, xend = 2, y = y_pos - y_max * 0.02, yend = y_pos - y_max * 0.02, color = "black") +
                annotate("segment", x = 1, xend = 1, y = y_pos - y_max * 0.02, yend = y_pos - y_max * 0.04, color = "black") +
                annotate("segment", x = 2, xend = 2, y = y_pos - y_max * 0.02, yend = y_pos - y_max * 0.04, color = "black") +
            ...
            if (show_main_symbol) {
              ...
              if (sig_symbol != "ns") {
                ...
                y_max <- max(data[[value_col]], na.rm = TRUE)
                y_pos <- y_max * 1.15  # Position higher to avoid overlap
          ...
          if (sig_symbol != "ns" && show_main_symbol) {
            ...
            if (exists("max_bracket_y") && !is.null(max_bracket_y) && max_bracket_y > y_axis_limit) {
              data_y_range <- max(data[[value_col]], na.rm = TRUE) - min(data[[value_col]], na.rm = TRUE)
              ...
              for (i in 1:(n_groups-1)) {
                for (j in (i+1):n_groups) {
                  ...
                  tryCatch({
                    ...
                    if (pairwise_sig != "ns") {
                      # Calculate positions for comparison line
                      x1 <- i
                      x2 <- j
                      line_y <- y_max * (1.05 + 0.04 * (j - i - 1))  # Stagger lines
                      ...
                      p <- p +
                        ...
                        annotate("segment", x = x1, xend = x1, y = line_y - y_max * 0.01, yend = line_y + y_max * 0.01, color = "black", linewidth = 0.5) +
                        annotate("segment", x = x2, xend = x2, y = line_y - y_max * 0.01, yend = line_y + y_max * 0.01, color = "black", linewidth = 0.5) +
                        annotate("text", x = (x1 + x2) / 2, y = line_y + y_max * 0.02, label = pairwise_sig, size = 6, hjust = 0.5)
                    }
                  }, error = function(e) {
    ...
    sato_transform_data <- function(dat, x_col=NULL, y_col=NULL, x_scale="linear", y_scale="linear") {
      ...
      if (!is.null(y_col) && y_scale == "-log10") {
        dat[[y_col]] <- -log10(as.numeric(dat[[y_col]]))
      } else if (!is.null(y_col) && y_scale == "-log2") {
        dat[[y_col]] <- -log2(as.numeric(dat[[y_col]]))
      }
    ...
    sato_apply_theme <- function(p, target_font, title_weight, axis_weight,
      ...
      p <- p + sato_theme(theme=theme_name, base_size=11, family=target_font,
                         title_face=title_weight, axis_face=axis_weight,
        ...
        theme(
          ...
          # Axis-related text
          axis.title = element_text(family = target_font, face = axis_weight),
          ...
          axis.text = element_text(family = target_font, color = "black"),
    ...
    sato_hist <- function(dat, x_col=1, bins=20, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
      ...
      sato_apply_theme(p, target_font, title_weight, axis_weight,
                      title_size, x_axis_title_size, y_axis_title_size, x_axis_text_size, y_axis_text_size, legend_text_size,
                      title_text, x_text, y_text, 
                      show_title, show_x_label, show_y_label,
                      x_scale, y_scale,
    ...
    sato_box <- function(dat, x_col=1, y_col=NULL, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
                        target_font="Arial", title_weight="plain", axis_weight="plain",
                        title_size=14, x_axis_title_size=12, y_axis_title_size=12, x_axis_text_size=10, y_axis_text_size=10, legend_text_size=16,
      ...
      if (is.null(y_col) || ncol(dat) == 1) {
        ...
        p <- ggplot(dat, aes(x="", y=dat[[x_col]])) +
             geom_boxplot(fill=fill, color=color, linewidth=linewidth, alpha=alpha,
        ...
        if (add_statistics && length(unique(plot_data$group)) > 1) {
          p <- sato_add_statistics_to_plot(p, plot_data, "group", "value", statistical_test, sato_symbol_size, posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)
        }
      }
    ...
    sato_line_grouped_error_raw <- function(dat, group_col=1, x_col=2, y_col=3,
                                            ...
                                            vbracket_timepoint="", vbracket_position="topright", vbracket_x=0.85, vbracket_y=0.85,
      ...
      if (add_statistics && n_groups >= 2) {
          ...
          if (nrow(data_at_x) > 0) {
            ...
            if (n_groups_at_x == 2) {
              ...
              if (length(group1_data) > 0 && length(group2_data) > 0) {
                ...
                if (length(group1_data) >= 3 && length(group1_data) <= 5000) {
                  ...
                  if (!is.null(shapiro_result1)) {
                    ...
                    normality_text <- c(normality_text,
                      sprintf("  %s: Shapiro-Wilk p=%.4f (%s)", groups_with_data[1], shapiro_result1$p.value, norm_status))
                ...
                if (length(group2_data) >= 3 && length(group2_data) <= 5000) {
                  ...
                  if (!is.null(shapiro_result2)) {
                    ...
                    normality_text <- c(normality_text,
                      sprintf("  %s: Shapiro-Wilk p=%.4f (%s)", groups_with_data[2], shapiro_result2$p.value, norm_status))
                ...
                if (test_to_use != "wilcoxon") {
                  if (variance_test == "levene") {
                    ...
                    if (!is.null(levene_result)) {
                      ...
                      variance_text <- sprintf("Variance (Levene): p=%.4f (%s)", levene_p, variance_status)
                    ...
                    if (!is.null(var_test)) {
                      equal_variances <- var_test$p.value > 0.05
                      variance_status <- if (equal_variances) "equal variances" else "unequal variances"
                      variance_text <- sprintf("Variance (F-test): p=%.4f (%s)", var_test$p.value, variance_status)
              ...
              for (grp in unique(anova_data$group)) {
                ...
                if (length(grp_data) >= 3 && length(grp_data) <= 5000) {
                  ...
                  if (!is.null(shapiro_result)) {
                    ...
                    normality_text <- c(normality_text,
                      sprintf("  %s: Shapiro-Wilk p=%.4f (%s)", grp, shapiro_result$p.value, norm_status))
              ...
              if (!is.null(anova_result)) {
                ...
                # Determine ANOVA significance
                anova_sig <- ""
                if (!is.na(anova_p)) {
                  if (anova_p < 0.001) anova_sig <- "***"
                  else if (anova_p < 0.01) anova_sig <- "**"
                  else if (anova_p < 0.05) anova_sig <- "*"
                  else anova_sig <- "ns"
                }

                # Store ANOVA result with normality info
                anova_text <- sprintf("Time point %.1f: ANOVA p=%.4f (%s)", x_val, anova_p, anova_sig)
                if (length(normality_text) > 0) {
                ...
                if (!is.na(anova_p) && anova_p < 0.05) {
                  ...
                  if (!is.null(tukey_result)) {
                    ...
                    for (i in 1:nrow(tukey_summary)) {
                      ...
                      sig_label <- ""
                      if (!is.na(p_adj)) {
                        if (p_adj < 0.001) sig_label <- "***"
                        else if (p_adj < 0.01) sig_label <- "**"
                        else if (p_adj < 0.05) sig_label <- "*"
                        else sig_label <- "ns"
                      } else {
                        sig_label <- "ns"
                      }
                  ...
                  anova_text <- paste0(anova_text, " (no post-hoc)")
                }
        ...
        # DEBUG: Add text annotation to plot to see values
        p <- p + annotate("text", x = Inf, y = Inf,
                         label = sprintf("DEBUG: n_groups=%d, stat_results=%d", n_groups, nrow(stat_results)),
        ...
        if (n_groups == 2 && nrow(stat_results) > 0) {
          ...
          # DEBUG annotation
          p <- p + annotate("text", x = Inf, y = Inf,
                           label = "DEBUG: In vbracket section (n>=3)",
                           hjust = 1.1, vjust = 3, size = 3, color = "blue")

          cat("Multi-group line plot (3+ groups): Using vbracket for statistical legend\\n")

          # Load vbracket functions by sourcing R files directly
          vbracket_loaded <- FALSE
          tryCatch({
            # Try loading as package first
            suppressPackageStartupMessages(library(vbracket))
            vbracket_loaded <- TRUE
          }, error = function(e1) {
            # Package not available, try sourcing from GitHub local copy
            vbracket_path <- "/Users/yoshiakisato/Desktop/github/vbracket_github/R"
            if (dir.exists(vbracket_path)) {
              tryCatch({
                source(file.path(vbracket_path, "helpers.R"))
                source(file.path(vbracket_path, "custom_legend_with_brackets.R"))
                source(file.path(vbracket_path, "annotation_grob.R"))
                source(file.path(vbracket_path, "ggplot_integration.R"))
                vbracket_loaded <<- TRUE
                p <<- p + annotate("text", x = Inf, y = Inf,
                                 label = "vbracket: sourced from files",
                                 hjust = 1.1, vjust = 4.5, size = 3, color = "green")
              }, error = function(e2) {
                p <<- p + annotate("text", x = -Inf, y = Inf,
                                 label = sprintf("vbracket source error:\n%s", substr(e2$message, 1, 60)),
                                 hjust = -0.05, vjust = 1.5, size = 3, color = "red")
              })
            } else {
              p <<- p + annotate("text", x = -Inf, y = Inf,
                               label = "vbracket not found:\nPackage not installed,\nlocal files not found",
                               hjust = -0.05, vjust = 1.5, size = 3, color = "red", lineheight = 0.9)
            }
          })
          ...
          if (vbracket_loaded) {
            tryCatch({
              ...
              if (length(stat_text_results) > 0) {
                ...
                for (result_text in filtered_results) {
                  ...
                  for (line in lines) {
                    ...
                    # Look for comparison lines with significance
              ...
              if (length(groups1) > 0) {
                ...
                # Add vbracket custom legend
                p <- p + theme(legend.position = "none")

                # Use custom position if specified, otherwise use preset position
                if (vbracket_position == "custom") {
                  cat(sprintf("Using custom vbracket position: x=%.2f, y=%.2f\\n", vbracket_x, vbracket_y))
                  p <- p + legend_bracket(
                    labels = group_labels,
                  ...
                  cat(sprintf("Using preset vbracket position: %s\\n", vbracket_position))
                  p <- p + legend_bracket(
                    labels = group_labels,
                    colors = legend_colors,
                    comparisons = comparisons_df,
              ...
              p <- p + annotate("text", x = Inf, y = Inf,
                               label = sprintf("vbracket error: %s", substr(e$message, 1, 50)),
                               hjust = 1.1, vjust = 6, size = 2.5, color = "red")
            })
          } else {
            p <- p + annotate("text", x = Inf, y = Inf,
                             label = "vbracket not loaded",
                             hjust = 1.1, vjust = 4.5, size = 3, color = "red")
    ...
    sato_bar_error_dot <- function(dat, x_col=1, y_col=2, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
      ...
      p <- ggplot() +
           ...
           geom_jitter(data = df, aes(x = category, y = value), 
                      width = jitter_width, size = dot_size, alpha = dot_alpha, color = dot_color, shape = dot_shape)
      ...
      if (add_statistics && length(unique(df$category)) > 1) {
        ...
        p <- sato_add_statistics_to_plot(p, df, "category", "value", statistical_test, sato_symbol_size, show_main_symbol=${showMainStatSymbol ? 'TRUE' : 'FALSE'}, show_pairwise=${showPairwiseComparisons ? 'TRUE' : 'FALSE'}, posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)
      } else {
        if (!add_statistics) {
    ...
    sato_box_dot <- function(dat, x_col=1, y_col=2, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
      ...
      p <- ggplot(df, aes(x = category, y = value)) +
           ...
           # Individual data points (jittered)
           geom_jitter(width = jitter_width, size = dot_size, alpha = dot_alpha, color = dot_color, shape = dot_shape)

      # Add statistical analysis if requested and we have multiple groups
      if (add_statistics && length(unique(df$category)) > 1) {
        p <- sato_add_statistics_to_plot(p, df, "category", "value", statistical_test, sato_symbol_size, posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)
      }
    ...
    sato_bar_grouped_error <- function(dat, group_col=1, x_col=2, y_col=3,
                                      fill_colors=c("#4C78A8", "#E15759"), stroke_color="#1f2937",
    ...
    sato_bar_grouped_error_dot <- function(dat, group_col=1, x_col=2, y_col=3,
      ...
      p <- ggplot(summary_data, aes(x = category, y = mean_val, fill = group)) +
           ...
           geom_errorbar(aes(ymin = mean_val - error_val, ymax = mean_val + error_val),
                        position = position_dodge(width = dodge_width),
                        width = 0.25, linewidth = linewidth * 0.8) +
           geom_jitter(data = plot_data, aes(x = category, y = value, fill = group),
      ...
      if (add_statistics && n_groups >= 2) {
        tryCatch({
          ...
          if (is.null(selected_categories) || length(selected_categories) == 0) {
            ...
            # Test only selected categories
            categories_to_test <- selected_categories
          }
            ...
            if (n_groups_at_cat == 2) {
              ...
              if (length(group1_data) > 0 && length(group2_data) > 0) {
                ...
                if (!is.null(test_result)) {
                  ...
                  # Store result text for UI with normality and variance info
                  result_text <- sprintf("Category %s: %s vs %s, %s p=%.4f (%s)",
                                        cat_val, groups_with_data[1], groups_with_data[2],
                                        test_name, p_val, sig_label)
                  if (length(normality_text) > 0) {
                    result_text <- paste0(result_text, "\\nNormality (Shapiro-Wilk):\\n", paste(normality_text, collapse="\\n"))
                  }
                  ...
                  comp_key <- paste0(groups_with_data[1], "-", groups_with_data[2], "@", cat_val)
                  if (!is.null(custom_y_positions[[comp_key]])) {
                    # Use custom Y position
                    y_pos <- custom_y_positions[[comp_key]]
                    cat("Using custom Y position for", comp_key, ":", y_pos, "\\n")
                  } else {
                    # Calculate default position
                    # Get the summary data for this category to find the bar heights
                    summary_at_cat <- summary_data[summary_data$category == cat_val, ]
                    max_mean_val <- max(summary_at_cat$mean_val, na.rm = TRUE)
                    max_error_val <- max(summary_at_cat$error_val, na.rm = TRUE)

                    # Position symbol above the tallest bar + error bar with some spacing
                    y_pos <- max_mean_val + max_error_val + (max_mean_val * 0.05)
                  }
              ...
              if (nrow(anova_data) >= 3) {
                ...
                # Build result text
                result_text <- sprintf("Category %s: %s p=%.4f", cat_val, test_name, omnibus_p)
                if (length(normality_text) > 0) {
                  result_text <- paste0(result_text, "\\nNormality (Shapiro-Wilk):\\n", paste(normality_text, collapse="\\n"))
                }
                ...
                if (!is.na(omnibus_p) && omnibus_p < 0.05) {
                  ...
                  if (test_to_use == "kruskal") {
                    ...
                    if (!is.null(posthoc_result)) {
                      ...
                      for (i in 1:nrow(posthoc_result)) {
                        ...
                        # Add significant comparisons to bracket data
                        if (sig_label != "ns") {
                  ...
                  # Combine omnibus and post-hoc results
                  sig_label <- if (omnibus_p < 0.001) "***" else if (omnibus_p < 0.01) "**" else "*"
                  result_text <- paste0(result_text, " (", sig_label, ")")
                  if (length(posthoc_text) > 0) {
                    ...
                    posthoc_header <- if (test_to_use == "kruskal") {
                      ...
                      test_name <- switch(selected_posthoc_test,
                        "tukey" = "Tukey HSD",
                      ...
                      paste0("Post-hoc pairwise comparisons (", test_name, "):")
                    }
                  ...
                  result_text <- paste0(result_text, " (ns)")
                  stat_text_results <- c(stat_text_results, result_text)
                }
              }
            }
        ...
        if (nrow(bracket_data) > 0) {
          ...
          y_range <- diff(range(c(summary_data$mean_val + summary_data$error_val,
                                 summary_data$mean_val - summary_data$error_val), na.rm = TRUE))
          ...
          for (cat_idx in unique(bracket_data$x.position)) {
            ...
            if (length(na_positions) > 0) {
              ...
              for (j in seq_along(na_positions)) {
                ...
                summary_at_cat <- summary_data[summary_data$category == cat_val, ]
                g1_summary <- summary_at_cat[summary_at_cat$group == g1, ]
                g2_summary <- summary_at_cat[summary_at_cat$group == g2, ]

                if (nrow(g1_summary) > 0 && nrow(g2_summary) > 0) {
                  # Match regular bar's baseHeight calculation:
                  # baseHeight = max of (error_bar_top, max_data_point) for each group
                  g1_error_top <- g1_summary$mean_val + g1_summary$error_val
                  g2_error_top <- g2_summary$mean_val + g2_summary$error_val

                  # Also get max data point for each group at this category
                  g1_data <- plot_data[plot_data$category == cat_val & plot_data$group == g1, "value"]
                  g2_data <- plot_data[plot_data$category == cat_val & plot_data$group == g2, "value"]
                  g1_max_data <- if(length(g1_data) > 0) max(g1_data, na.rm = TRUE) else g1_error_top
                  g2_max_data <- if(length(g2_data) > 0) max(g2_data, na.rm = TRUE) else g2_error_top

                  # Top = max(error_top, max_data)
                  g1_top <- max(g1_error_top, g1_max_data, na.rm = TRUE)
                  g2_top <- max(g2_error_top, g2_max_data, na.rm = TRUE)

                  baseHeight <- max(g1_top, g2_top, na.rm = TRUE)
                } else {
                  # Fallback: use max of all groups at this category
                  baseHeight <- max(summary_at_cat$mean_val + summary_at_cat$error_val, na.rm = TRUE)
                }

                # Calculate Y position with stacking
                if (j == 1) {
                  # First bracket at this category (higher to avoid overlap)
                  bracket_data$y.position[bracket_idx] <- baseHeight + (unit_step * 3.0)
                } else {
                  ...
                  bracket_data$y.position[bracket_idx] <- max(
                    baseHeight + (unit_step * 4.0),
                    bracket_data$y.position[prev_bracket_idx] + (unit_step * 4.0)
                  )
                }
              }
            }
        ...
        if (nrow(bracket_data) > 0) {
          ...
          for (i in 1:nrow(bracket_data)) {
            if (!is.na(bracket_data$group1[i]) && bracket_data$group1[i] != "" &&
              ...
              if (length(g1_idx) > 0 && length(g2_idx) > 0) {
                # Calculate X positions using position_dodge logic
                ...
                bracket_data$xmin[i] <- g1_x
                bracket_data$xmax[i] <- g2_x
              }
            }
          }
    ...
    sato_violin <- function(dat, x_col=1, y_col=2, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
                           target_font="Arial", title_weight="plain", axis_weight="plain",
                           title_size=14, x_axis_title_size=12, y_axis_title_size=12, x_axis_text_size=10, y_axis_text_size=10, legend_text_size=16,
      ...
      p <- ggplot(df, aes(x = category, y = value)) +
           geom_violin(fill = fill, color = color, linewidth = linewidth, alpha = alpha)
    ...
    sato_violin_dot <- function(dat, x_col=1, y_col=2, fill="#4C78A8", color="#1F2937", linewidth=0.7, alpha=0.9,
                               dot_size=4, dot_alpha=1.0, dot_color="#333333", dot_shape=16, jitter_width=0.2,
                               target_font="Arial", title_weight="plain", axis_weight="plain",
                               title_size=14, x_axis_title_size=12, y_axis_title_size=12, x_axis_text_size=10, y_axis_text_size=10, legend_text_size=16,
...
function detectHeader(rows) {
  ...
  return (r1Ratio >= 0.5 && r2Ratio >= 0.5); // シンプルで堅い
}
  ...
  return Excel.run(async (ctx) => {
    ...
    try {
      ...
      if (!sel.values || sel.values.length === 0) {
        throw new Error("選択されたセル範囲にデータがありません。データを含むセル範囲を選択してください。");
      }
      ...
      if (!pinnedRange) throw new Error("セル範囲を選択して『読み込み』してください。データを含むセル範囲を選択してからボタンを押してください。");
      ...
      if (!rng.values || rng.values.length === 0) {
        throw new Error("ピン留めされた範囲にデータがありません。新しいデータ範囲を選択して『読み込み』を実行してください。");
      }
...
async function cacheDataFromSelection(headerMode = "auto") {
  ...
  if (!values || values.length === 0) throw new Error("選択範囲が空です。");
  ...
  else header = detectHeader(values);
  ...
  if (!csv || csv.trim().length === 0) {
    throw new Error("CSVデータが空です。有効なデータを含むセル範囲を選択してください。");
  }
  ...
  if (!webR || !webrReady) {
    throw new Error("WebRの初期化に失敗しました。");
  }
...
const fontStack = buildCompleteFontStack(effectiveFont); 
  ...
  // Calculate column indices for grouped charts
  ...
  let groupColIndex = 1, xColIndex = 2, yColIndex = 3, errorColIndex = 4;
  ...
  if (GROUPED_CHART_TYPES.includes(chartType) && window.lastProcessedData && window.lastProcessedData.length > 0) {
    const headers = window.lastProcessedData[0];
    ...
    console.log("  ⚠️ Using default indices (1,2,3,4) because condition failed");
  }
  ...
  const getDefaultYLabel = (type) => {
    switch(type) {
      ...
      case 'bar': return 'Count';
      case 'bar_error': return 'Mean';
  ...
  const axisWeight = o.axisWeight === 'bold' ? 'bold' : 'plain';
  ...
  // VBracket legend settings (for 3+ groups line plots)
  const vbracketPosition = o.vbracketPosition || "topright";
  const vbracketX = Number(o.vbracketX) || 0.85;
  const vbracketY = Number(o.vbracketY) || 0.85;
  const vbracketTextSize = Number(o.vbracketTextSize) || 10;
  const vbracketMargin = Number(o.vbracketMargin) || 0.15;
  const vbracketLineWidth = Number(o.vbracketLineWidth) || 0.5;
  console.log("VBracket settings:", { vbracketPosition, vbracketX, vbracketY, vbracketTextSize, vbracketMargin, vbracketLineWidth });
  ...
  const plotCode = `
    ...
    if (data_order == "original") {
      ...
      target_col <- if(chart_type == "bar_grouped") 2 else 1  # Column 2 for grouped bar, Column 1 for others
    ...
    if (chart_type == "histogram") {
      ...
      p <- sato_box(
        dat = dat,
        x_col = 1,
        y_col = if(ncol(dat) > 1) 2 else NULL,
        fill = "${fillColor}",
        color = "${strokeColor}",
        alpha = ${fillAlpha},
        linewidth = ${lineWidth},
      ...
      # ADD MANUAL STATISTICS OVERRIDE FOR BOX
      if (TRUE) {
        p <- sato_add_statistics_to_plot(p, dat, 1, 2, '${statisticalTest}', sato_symbol_size, show_main_symbol=${showMainStatSymbol ? 'TRUE' : 'FALSE'}, show_pairwise=${showPairwiseComparisons ? 'TRUE' : 'FALSE'}, ggpubr_symbol_size=${statSymbolSize}, ggpubr_line_size=${statLineSize}, ggpubr_tip_length=${statTipLength}, ggpubr_vjust=${statVjust}, comparison_mode="${comparisonMode}", custom_comparisons='${JSON.stringify(customComparisons).replace(/'/g, "\\'")}', custom_positions='${JSON.stringify(customPositions).replace(/'/g, "\\'")}', posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)
      }
      ...
      # ADD MANUAL STATISTICS OVERRIDE FOR BOX_DOT
      if (TRUE) {
        p <- sato_add_statistics_to_plot(p, dat, 1, 2, '${statisticalTest}', sato_symbol_size, show_main_symbol=${showMainStatSymbol ? 'TRUE' : 'FALSE'}, show_pairwise=${showPairwiseComparisons ? 'TRUE' : 'FALSE'}, ggpubr_symbol_size=${statSymbolSize}, ggpubr_line_size=${statLineSize}, ggpubr_tip_length=${statTipLength}, ggpubr_vjust=${statVjust}, comparison_mode="${comparisonMode}", custom_comparisons='${JSON.stringify(customComparisons).replace(/'/g, "\\'")}', custom_positions='${JSON.stringify(customPositions).replace(/'/g, "\\'")}', posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)
      }
      ...
      # ADD MANUAL STATISTICS OVERRIDE FOR VIOLIN
      if (TRUE) {
        p <- sato_add_statistics_to_plot(p, dat, 1, 2, '${statisticalTest}', sato_symbol_size, show_main_symbol=${showMainStatSymbol ? 'TRUE' : 'FALSE'}, show_pairwise=${showPairwiseComparisons ? 'TRUE' : 'FALSE'}, ggpubr_symbol_size=${statSymbolSize}, ggpubr_line_size=${statLineSize}, ggpubr_tip_length=${statTipLength}, ggpubr_vjust=${statVjust}, comparison_mode="${comparisonMode}", custom_comparisons='${JSON.stringify(customComparisons).replace(/'/g, "\\'")}', custom_positions='${JSON.stringify(customPositions).replace(/'/g, "\\'")}', posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)
      }
      ...
      # ADD MANUAL STATISTICS OVERRIDE FOR VIOLIN_DOT
      if (TRUE) {
        p <- sato_add_statistics_to_plot(p, dat, 1, 2, '${statisticalTest}', sato_symbol_size, show_main_symbol=${showMainStatSymbol ? 'TRUE' : 'FALSE'}, show_pairwise=${showPairwiseComparisons ? 'TRUE' : 'FALSE'}, ggpubr_symbol_size=${statSymbolSize}, ggpubr_line_size=${statLineSize}, ggpubr_tip_length=${statTipLength}, ggpubr_vjust=${statVjust}, comparison_mode="${comparisonMode}", custom_comparisons='${JSON.stringify(customComparisons).replace(/'/g, "\\'")}', custom_positions='${JSON.stringify(customPositions).replace(/'/g, "\\'")}', posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)
      }
      ...
      p <- sato_bar_error(
        ...
        x_col = 1,
        y_col = if(ncol(dat) > 1) 2 else NULL,
        error_col = if(ncol(dat) > 2) 3 else NULL,
        fill = "${fillColor}",
        color = "${strokeColor}",
        alpha = ${fillAlpha},
        linewidth = ${lineWidth},
      ...
      # ADD MANUAL OVERRIDE ANNOTATION FOR BAR_ERROR_DOT
      # FORCE STATISTICS FOR TESTING
      cat("\\n\\n🔥🔥🔥 BAR_ERROR_DOT: FORCING STATS ADDITION 🔥🔥🔥\\n")

      if (TRUE) {  # TEMPORARILY FORCE TO TRUE FOR TESTING
        cat("🔥🔥🔥 CONDITION PASSED - CALLING STAT FUNCTION 🔥🔥🔥\\n")
        cat("🔥🔥🔥 ADDING MANUAL OVERRIDE ANNOTATION USING IMPROVED FUNCTION 🔥🔥🔥\\n")
        cat("🔥🔥🔥 ABOUT TO CALL sato_add_statistics_to_plot with parameters: p, dat, 1, 2, '${statisticalTest}', sato_symbol_size=", sato_symbol_size, "🔥🔥🔥\\n")

        # CALL THE STATISTICAL FUNCTION
        p <- sato_add_statistics_to_plot(p, dat, 1, 2, '${statisticalTest}', sato_symbol_size, show_main_symbol=${showMainStatSymbol ? 'TRUE' : 'FALSE'}, show_pairwise=${showPairwiseComparisons ? 'TRUE' : 'FALSE'}, ggpubr_symbol_size=${statSymbolSize}, ggpubr_line_size=${statLineSize}, ggpubr_tip_length=${statTipLength}, ggpubr_vjust=${statVjust}, comparison_mode="${comparisonMode}", custom_comparisons='${JSON.stringify(customComparisons).replace(/'/g, "\\'")}', custom_positions='${JSON.stringify(customPositions).replace(/'/g, "\\'")}', posthoc_test=selected_posthoc_test, dunnett_control=dunnett_control)

        cat("🔥🔥🔥 STATISTICS ADDED VIA sato_add_statistics_to_plot FUNCTION 🔥🔥🔥\\n")

        # REMOVED OLD DUPLICATE CODE - All statistics now handled by sato_add_statistics_to_plot function above
        # (Deleted ~185 lines of old bracket positioning code that was overriding the correct function)
      } else {
        cat("🔥🔥🔥 MANUAL OVERRIDE NOT TRIGGERED - CONDITIONS NOT MET 🔥🔥🔥\\n")
      }
      ...
      # Grouped bar chart with error - requires 3 columns: Group, Category, Value
  ...
  try {
    ...
    if (!svgText || svgText.length < 100) {
      throw new Error(`SVG生成失敗: 長さ ${svgText.length}`);
    }
...
function formatTextForR(text) {
  ...
  console.log(`DEBUG: Processing text for R: "${trimmed}"`);
  ...
  console.log(`DEBUG: Plain text: "${escapedText}"`);
  return `"${escapedText}"`;
}
...
function uiOpts(){
  ...
  return {
    ...
    showMainStatSymbol: el("showMainStatSymbol")?.checked || false,
    ...
    // VBracket legend settings (for 3+ groups line plots)
    vbracketPosition: el("vbracketPosition")?.value || "topright",
    vbracketX: Number(el("vbracketX")?.value) || 0.85,
    vbracketY: Number(el("vbracketY")?.value) || 0.85,
    vbracketTextSize: Number(el("vbracketTextSize")?.value) || 10,
    vbracketMargin: Number(el("vbracketMargin")?.value) || 0.15,
    vbracketLineWidth: Number(el("vbracketLineWidth")?.value) || 0.5,
  };
}
...
async function previewPlotWithDebug() {
  try {
    setStatus("Generating preview with font debug...");
    ...
    img.onload = () => {
      ...
      setStatus(
        `プレビュー完了: ${m.width}${m.units} × ${m.height}${m.units}\n` +
        `フォント: ${m.font.ui} -> ${m.font.effective}` +
        (m.font.loaded ? " (成功)" : " (fallback)")
      );
    ...
    setStatus("プレビューエラー: " + (e?.message || e));
  }
}
...
async function insertIntoExcelFixed() {
  try {
    ...
    setStatus(
      `✅ Excel挿入完了 (with metadata): ${meta.width}${meta.units} × ${meta.height}${meta.units}\n` +
      `フォント: ${fontInfo.ui} -> ${fontInfo.r} (${fontInfo.loaded ? "OK" : "フォールバック"})\n\n` +
      `To save: Right-click image → "Save as Picture..."`
    );

  } catch (e) {
    console.error("Insert error:", e);
    setStatus("Excel挿入エラー: " + (e?.message || e));
  }
}