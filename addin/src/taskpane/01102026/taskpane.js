...
async function writeRCodeToCell(rCode) {
  try {
    console.log('📝 Writing R code to Excel - first 500 chars:', rCode.substring(0, 500));
    console.log('📝 Check labs line in code:', rCode.match(/labs\(title.*?\)/)?.[0]);
...
# Add significance brackets to plot using annotate
`;

  // Generate bracket annotation code for each pair
  const yColIndex = settings.yColIndex - 1;
  const xColIndex = settings.xColIndex - 1;
  const allYValues = window.lastProcessedData
    .slice(1)
  ...
  pairs.forEach(([cat1, cat2], pairIdx) => {
    ...
    const cat1Idx = uniqueCategories.indexOf(cat1) + 1; // R 1-based
    const cat2Idx = uniqueCategories.indexOf(cat2) + 1;
    ...
    // Calculate Y position: either custom or automatic (stack multiple brackets)
    let yBracketPos;
    ...
    // Add conditional or unconditional bracket based on comparison mode
    const showAll = settings.comparisonMode === 'all' || settings.comparisonMode === 'custom';

    if (showAll) {
      // Show all comparisons regardless of significance
      const posComment = customYPos !== undefined && customYPos !== null
        ? ' # Custom Y position from user setting'
        : ' # Auto-calculated Y position';
      code += `
# Add bracket for ${cat1} vs ${cat2}
y_bracket_${pairName} <- ${yBracketPos}${posComment}

# Draw bracket lines
p <- p + annotate('segment', x = ${cat1Idx}, xend = ${cat1Idx},
                  y = y_bracket_${pairName} - ${(yRange * 0.02).toFixed(2)}, yend = y_bracket_${pairName},
                  linewidth = ${settings.lineWidth || 0.7}, color = 'black')
p <- p + annotate('segment', x = ${cat1Idx}, xend = ${cat2Idx},
                  y = y_bracket_${pairName}, yend = y_bracket_${pairName},
                  linewidth = ${settings.lineWidth || 0.7}, color = 'black')
p <- p + annotate('segment', x = ${cat2Idx}, xend = ${cat2Idx},
                  y = y_bracket_${pairName} - ${(yRange * 0.02).toFixed(2)}, yend = y_bracket_${pairName},
                  linewidth = ${settings.lineWidth || 0.7}, color = 'black')

# Add significance text above bracket
p <- p + annotate('text', x = ${(cat1Idx + cat2Idx) / 2}, y = y_bracket_${pairName} + ${(yRange * 0.02).toFixed(2)},
                  label = sig_${pairName}, size = ${settings.statSymbolSize || 7})
`;
    } else {
      ...
      code += `
# Add bracket for ${cat1} vs ${cat2}
if (sig_${pairName} != 'ns') {  # Only show significant comparisons
  y_bracket_${pairName} <- ${yBracketPos}${posComment}
...
`;
  ...
  if (settings.comparisonMode === 'all') {
    code += `
# Note: All comparisons are shown on the plot (including non-significant 'ns').
# To show only significant comparisons (p < 0.05), add 'if (sig != 'ns')' condition.

`;
  } else if (settings.comparisonMode === 'custom') {
    const hasCustomPos = Object.keys(customPositions).length > 0;
    code += `
# Note: Only selected custom comparisons are shown on the plot.
# These are the comparisons you selected in the add-in interface.`;
    if (hasCustomPos) {
      code += `
